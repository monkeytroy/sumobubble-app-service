                ...
                return (
                  <div key={section.name} className="divide-y divide-gray-200 rounded-lg bg-white shadow flex flex-col">
                    <div className="grow flex w-full items-center justify-between space-x-6 p-6">
                      ...
                      <div className="h-10 w-10 flex-shrink-0 rounded-full text-blue-400">{section.icon}</div>
                    </div>
                    <div className="grow-0">
                      <div className="-mt-px flex divide-x divide-gray-200">
                        <div
                          className="flex w-0 flex-1 px-2 py-4 items-center justify-center cursor-pointer hover:bg-gray-100"
                          onClick={() => onSectionClick(section)}
                          title="Click to enable or disable">
                          {!!configuration?.sections[section.name.toLowerCase() as keyof ISiteSections]?.enabled &&
                            'Enabled'}
                          {!!!configuration?.sections[section.name.toLowerCase() as keyof ISiteSections]?.enabled &&
                            'Disabled'}
                        </div>
